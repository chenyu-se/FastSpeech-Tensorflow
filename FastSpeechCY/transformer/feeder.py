...
from transformer.infolog import log
...
class Feeder:
	...
	def __init__(self, coordinator, metadata_filename, hparams):
		...
		self._alignment_dir = os.path.join(os.path.dirname(metadata_filename), 'alignment')
		...
		self._pad = 0
		self._alignment_pad = 0
		#explicitely setting the padding to a value that doesn't originally exist in the spectogram
		#to avoid any possible conflicts, without affecting the output range of the model too much
		if hparams.symmetric_mels and False:
		...
		with tf.device('/cpu:0'):
            ...
            tf.placeholder(tf.float32, shape=(None, None), name='alignment_targets'),
			...
			self.inputs, self.input_lengths, self.mel_targets, self.token_targets, self.linear_targets, self.alignment_targets, self.targets_lengths = queue.dequeue()
			...
			self.alignment_targets.set_shape(self._placeholders[5].shape)
			...
			self.eval_inputs, self.eval_input_lengths, self.eval_mel_targets, self.eval_token_targets, \
				self.eval_linear_targets, self.eval_alignment_targets, self.eval_targets_lengths = eval_queue.dequeue()
			...
			self.eval_alignment_targets.set_shape(self._placeholders[5].shape)
	...
	def _get_next_example(self):
		...
		token_target = np.asarray([0.] * (len(mel_target) - 1))
		linear_target = np.load(os.path.join(self._linear_dir, meta[2]))
		alignment_target = np.load(os.path.join(self._alignment_dir, meta[6]))
		return (input_data, mel_target, token_target, linear_target, alignment_target, len(mel_target))
	...
	def _prepare_batch(self, batch, outputs_per_step):
		...
		alignment_targets = self._prepare_alignment_targets([x[4] for x in batch])
		targets_lengths = np.asarray([x[-1] for x in batch], dtype=np.int32) #Used to mask loss
		return (inputs, input_lengths, mel_targets, token_targets, linear_targets, alignment_targets, targets_lengths)
	...
	def _prepare_alignment_targets(self, targets):
		max_len = max([len(t) for t in targets]) + 1
		return np.stack([self._pad_alignment_target(t, max_len) for t in targets])
	...
	def _pad_alignment_target(self, t, length):
		return np.pad(t, (0, length - t.shape[0]), mode='constant', constant_values=self._alignment_pad)
